...
export default function ApplicationsTable({ data }) {
  ...
  useEffect(() => {
    ...
    if (positionCategory !== "All") {
      filtered = filtered.filter((d) => d.position_name === positionCategory);
    }
          ...
          {filteredData.map((data) => (
            ...
            >
              ...
              <TableCell className="text-right">
                <p
                  className={cn(
                    data.status === "Accepted" && "bg-emerald-600 text-white",
                    data.status === "Rejected" && "bg-red-500/70 text-white",
                    data.status === "Done" &&
                ...
                >
                  {data.status === "Pending" && <Loader size={11} />}
